...
in
{
  ...
  mkHost = profileName: hostConfigPath: hostParams:
    let
      profile = profiles.${profileName};
      
      # Get locale configuration (default to german-en if not specified)
      localeConfig = if hostParams ? "localeProfile"
                    then localeProfiles.getLocaleConfig hostParams.localeProfile "german-en"
                    else if hostParams ? "customLocale" 
                    then hostParams.customLocale
    ...
    in
    {
      ...
      networking.hostId = if hostParams ? "hostId" then hostParams.hostId 
                         else builtins.substring 0 8 (builtins.hashString "md5" hostParams.hostname);
      
      # Merge any additional configuration
    } // localeConfig // (hostParams.extraConfig or {}) // (profile.extraConfig or {});
}